...
class GameTrial(BaseModel):
    ...
    # Accept both snake_case (rt_raw) and camelCase (rtRaw) from Android clients
...
class GameSessionRequest(BaseModel):
    ...
    def check_data_provided(self):
        """Ensure at least one data format is provided"""
        if (self.trials is None or len(self.trials) == 0) and self.summary is None:
            raise ValueError(
                "Either 'trials' (list of trial objects) or 'summary' (aggregated metrics) must be provided. "
                "Trial fields: rtRaw (float >0), correct (0 or 1), error (0 or 1), hintUsed (0 or 1). "
                "Summary fields: totalAttempts, correct, errors, meanRtRaw."
            )
        return self
